 ...
 * Param
 * -----
 * top = North || space separated list with all of them
 * right = East
 * bottom = South
 * left = West
 */
Geomap.prototype.zoomAOI = function( top, right, bottom, left ) {
	...
	/*
	//
	// The following is to hide the feature that is outside the extent,
	// currently that feature is glitchy
	// * I can not monitor when the view of the map change, in order to remove the hidden style like
	// 		- if the user press the house button "Fit to extent".
	//		- if the user move the map
	//		- if the user change the zoom of the map
	// * The hidden feature are arbitrary to the extent. That means some feature inside are hidden and some feature outside (but near by) are still show
	// * When changing through multiple extent, the feature and the intersect do not behave as it should.
	// * When I did some test with the KML for the mine (as provided in the wet working example) an error get through in the console, and the feature don't get hidden, but after doing it twice it works. I think there is a possible issue with the KML file or with the configuration file. Idem with the TopoJSON (World 110m).
	//

	// Move this style with the other style in this library
	var styleTransparent = new ol.style.Style( {
		fill: new ol.style.Fill({
			color: "rgba(0,0,0,0)"
		})
	});

	if ( ! _this.arrFeatSChg ) {
		_this.arrFeatSChg = [];
	}
	arrFeatSChg = _this.arrFeatSChg;

	// Hide visible titled feature outside the extent
	OLmap.getLayers().forEach( function( lyr ) {
		// if ( lyr.getVisible() && lyr.title ) {
		if ( lyr.title ) {
			...
			try {
				lyr.getSource().forEachFeature( function( feature ) {
					...
					// if feature are inside the extent
					var _featGeo = feature.getGeometry();

					if ( !_featGeo.intersectsExtent( extent ) ) {

						// This feature are not inside the extent
						arrFeatSChg.push( [ feature, _featGeo, feature.getStyle() ] );

						feature.setStyle( styleTransparent );
					}
				});
			} catch( ex ) { }
		}
	...
	// When the zoom on the map change
	// Restore the feature style transparent
	var restoreFeature = function(event){
		var i,
			i_len = arrFeatSChg.length,
			tempArray = [],
			i_cache, feature, featureGeo,
			currentExtent = OLmap.getView().calculateExtent( OLmap.getSize() );

		for( i=0; i !== i_len; i = i +1 ) {
			i_cache = arrFeatSChg[ i ];
			feature = i_cache[ 0 ];
			featureGeo = i_cache[ 1 ];


			// // Reset if in the extent (Need the featureGeo)
			// if ( feature.getGeometry().intersectsExtent( currentExtent ) ) {
			// 	i_cache[ 0 ].setStyle( i_cache[ 2 ] );
			// } else {
			// 	tempArray.push( i_cache );
			// }
		...
		arrFeatSChg = tempArray;
	}

	// OLmap.on( 'change:view', function( event ) {
	//	console.log( event );
	// } );

	//OLmap.getView().on( "change:resolution", restoreFeature );
	setTimeout( restoreFeature, 3000 );
	*/
};
